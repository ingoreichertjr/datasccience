...
# Construindo um gráfico em camadas, utilizando a gramática dos gráficos
...
# No entanto, tambem podemos mapear uma variavel contínua à cor dos pontos:
View(mtcars)
ggplot(mtcars, aes(x = disp, y = mpg, colour = cyl)) + geom_point()

# Também podemos mapear o tamanho dos ponts à uma variável de interesse:
# A legenda é inserida no gráfico automaticamente
ggplot(mtcars, aes(x = disp, y = mpg, colour = cyl, size = wt)) + geom_point()

# Os geoms definem qual forma geometrica será utilizada para a visualização dos dados no gráfico
...
ggplot(mtcars, aes(x = mpg, y = disp, colour = as.factor(cyl))) +
  geom_point() +
  facet_grid(am~.) # significa construir uma relação entra a variavel am e todas as outras

ggplot(mtcars, aes(x = mpg, y = disp, colour = as.factor(cyl))) +
  geom_point() +
  facet_grid(.~am) # inverte com o gráfico anterior, muda o sentido
